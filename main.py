...
def check_file(file_path, local_root, target_url, match_codes, filter_content):
    ...
    try:
        ...
        if response.status_code in match_codes:
            if response.status_code == 200:
                print(f"{GREEN}[{response.status_code}] Found: {full_url}{RESET}")
            elif response.status_code == 403:
                print(f"{ORANGE}[{response.status_code}] Forbidden: {full_url}{RESET}")
            else:
    ...
    try:
        for root, dirs, files in os.walk(local_dir):
            for file in files:
                ...
                check_file(file_path, local_dir, target_url, args.match_codes, args.filter_content)
